    ...
    setup() {
      ...
      const fetchSentTreats = async () => {
        try {
          ...
          const treatsSent = userSnap.exists() ? userSnap.data().treatsSent || [] : [];
          ...
          for (let i = 0; i < treatsSent.length; i += 10) {
            ...
            const petQuery = query(collection(db, "Pet_Listings"), where("__name__", "in", chunk));
          ...
          petList.value = petDocs.map((doc) => {
            ...
            return {
              ...
              timeAgo: petData.createdAt ? formatTimeAgo(petData.createdAt) : "N/A",
            };
          });
        } catch (err) {
